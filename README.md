...
- **Formatriktig utdata** — `7S-rapport`-markdown (fritext-Händelse, MGRS-rutor med
  mellanslag, `signal_*`-frontmatter och `## Bilagor`-wikilänkar för bilder) som
  matchar formatet på rapporter insamlade av källappen.
...
`Sagesman`, `Sedan`). När `--images` används läggs en `## Bilagor`-sektion till med en
Obsidian-wikilänk (`![[<meddelandemapp>/bild.jpg]]`); bilden sparas i en per-meddelande-
mapp bredvid rapporterna (`<signaltid>_<DDHHMM>-<avsändare>/`), och skylten skrivs även
in i JPEG-kommentaren (markören `7SPLATE:`) så att en offline-konsument kan läsa den utan
OCR. Formatet speglar rapporter insamlade av källappen. Rapportrenderingen finns i
